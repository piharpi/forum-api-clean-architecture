...
describe('ReplyRepositoryPostgres', () => {
  ...
  describe('checkIsReplyAvailable function',  () => {
    it('should throw not found error when reply is not available', async () => {
      ...
      // Action & Assert
      await expect(replyRepositoryPostgres.checkIsReplyAvailable('reply-123')).rejects.toThrowError(NotFoundError);
    });
    ...
    it('should resolve when reply is available', async () => {
      ...
      // Action & Assert
      await expect(replyRepositoryPostgres.checkIsReplyAvailable('reply-123')).resolves.not.toThrowError();
    });
  })
  ...
  describe('getAllDetailReplyByThreadAndCommentId function', () => {
    it('should return detail reply correctly', async () => {
      ...
      await RepliesTableTestHelper.addReply({ id: 'reply-2', owner: 'user-1234'})

      const replyRepository = new ReplyRepositoryPostgres(pool, {});

      // Action
      const detailReply = await replyRepository.getAllDetailReplyByThreadAndCommentId('thread-123', 'comment-123');
